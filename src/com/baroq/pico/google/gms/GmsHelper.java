package com.baroq.pico.google.gms;
...
public class GmsHelper implements GooglePlayServicesClient.ConnectionCallbacks, GooglePlayServicesClient.OnConnectionFailedListener {

    /** Listener for sign-in success or failure events. */
    public interface GmsHelperListener {
        /**
    ...
    */
    ...
    // Listener
    GmsHelperListener mListener = null;

    /**
    * Construct a GmsHelper object, initially tied to the given Activity.
    ...
    */
    ...
    boolean checkState(int type, String operation, String warning, int... expectedStates) {
        ...
        if (type == TYPE_DEVELOPER_ERROR) {
            sb.append("GmsHelper: you attempted an operation at an invalid. ");
        } else {
            sb.append("GmsHelper: bug detected. Please report it at our bug tracker ");
    ...
    void assertConfigured(String operation) {
        if (mState == STATE_UNCONFIGURED) {
            String error = "GmsHelper error: Operation attempted without setup: " + operation +
    ...
    /**
    * Same as calling @link{setup(GmsHelperListener, int)} requesting only the
    * CLIENT_GAMES client.
    */
    public void setup(GmsHelperListener listener) {
        setup(listener, CLIENT_GAMES);
    }

    /**
    * Performs setup on this GmsHelper object. Call this from the onCreate()
    ...
    */
    public void setup(GmsHelperListener listener, int clientsToUse, String ... additionalScopes) {
        if (mState != STATE_UNCONFIGURED) {
            String error = "GmsHelper: you called GmsHelper.setup() twice. You can only call " +
            "it once.";
            logError(error);
            throw new IllegalStateException(error);
        }
    ...
    */
    ...
    public void showAlert(String title, String message) {
        debugLog("Alert Title: "+title+" msg: " + message);
    }

    /** Convenience method to show an alert dialog. */
    public void showAlert(String message) {
        debugLog("Alert msg: " + message);
    }

    /**
    * Returns the invitation ID received through an invitation notification.
    * This should be called from your GmsHelperListener's
    *
    * @link{GmsHelperListener#onSignInSucceeded} method, to check if there's an
    ...
    void notifyListener(boolean success) {
        ...
        if (mListener != null) {
            if (success) {
                ...
                mListener.onSignInFailed(mSignInFailureReason != null ? mSignInFailureReason.toString() : "FAILURE (no reason)");
            }
        }
    }
    ...
    * may show up. At the end of the process, the GmsHelperListener's
    ...
    */
    ...
    void connectCurrentClient() {
        if (mState == STATE_DISCONNECTED) {
            ...
            logWarn("GmsHelper got disconnected during connection process. Aborting.");
            return;
        }
    ...
    */
    public void reconnectClients(int whatClients) {
        ...
        if ((whatClients & CLIENT_PLUS) != 0 && mPlusClient != null
            ...
            logWarn("GmsHelper is ignoring your request to reconnect " +
    ...
    */
    ...
    void debugLog(String message) {
        if (mDebugLog) {
            Log.d(mDebugTag, "GmsHelper: " + message);
        }
    }

    void logWarn(String message) {
        Log.w(mDebugTag, "!!! GmsHelper WARNING: " + message);
    }

    void logError(String message) {
        Log.e(mDebugTag, "*** GmsHelper ERROR: " + message);
    }
